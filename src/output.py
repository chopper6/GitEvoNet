...
def parallel_configs(ID, output_dir, pressure, tolerance, pop_size, mutation_freq, percent_survive, start_size, grow_freq, crossover_percent):
    ...
    title = "Pressure, Tolerance, Mutation Frequency, Crossover Percent\n"
    ...
    with open(output_dir + "/" + str(ID) + "/worker_configs.csv", 'w') as outfile:
        outfile.write(title)
        outfile.write(str(pressure) + "," + str(tolerance) + "," + str(mutation_freq) + "," + str(crossover_percent))
...
def init_csv(pop_size, num_workers, out_dir, configs):
    ...
    csv_title = "Net ID#, Net Size, Fitness, Red-Green to Grey Ratio, Effective Total Benefits, Average Degree\n"
    csv_popn_title = "Net Size, Best Red-Green to Grey Ratio, Worst Red-Green to Grey Ratio, Best Effective Total Benefits, Worst Effective Total Benefits\n"
