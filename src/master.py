...
from operator import attrgetter, from time import process_time as ptime, from random import SystemRandom as sysRand
...
def evolve_master(configs):
    ...
    while (size < end_size):
        ...
        for g in range(master_gens):
            ...
            pool.join()
            pool.close()  #after join may throw an exception
            pool.terminate()
